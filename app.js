...
function buildFYData(rows){
  return rows.map(r => {
    ...
    return {
      ...
      workpoint:   (pickFieldCI(r,'Excel/ Workpoint','Excel/Workpoint','ExcelWorkpoint','Excel / Workpoint','Workpoint')||'').toString().trim(),
...
function monthFilter(m){
  ...
  if(Array.isArray(f)) return f.length === 0 || f.includes(m);
  return m === f;
}
...
function initRevFilters(){
  ...
  buildMultiSelect('#rev-month-filter-wrap', months, STATE.revMonth,
    val => { STATE.revMonth = val; renderRMRev(); }, {label:'Period'});
}
...
function topKPIs(){
  ...
  const generated = isAllMonths() ? rows.length : rows.filter(r=>monthFilter(r.CTM)).length;
  const sc = st => {
    if(isAllMonths()) return rows.filter(r=>r.leadStatus===st).length;
    return rows.filter(r => r.leadStatus===st && monthFilter(r[statusMonthCol(st)])).length;
  };
  ...
  for(const g of groups){
    ...
    for(const st of STATUSES){
      ...
      if(!isAllMonths()){
        ...
        if(mode==='AnyMonth'){
          pool = pool.filter(r => monthFilter(r[col]) && !isAllMonths() && monthKey(r.CTM) < Math.min(...(Array.isArray(STATE.filterMonth)?STATE.filterMonth:[STATE.filterMonth]).map(m=>monthKey(m))));
        } else if(mode==='SameMonth'){
          pool = pool.filter(r => monthFilter(r[col]) && monthFilter(r.CTM));
        } else {
          pool = pool.filter(r => monthFilter(r[col]));
        }
      }
...
function teamPerformance(){
  ...
  return teams.map(team => {
    ...
    const totalLeads = isAllMonths() ? rows.length : rows.filter(r => monthFilter(r.CTM)).length;
    const obj = {Team: team, 'Total Leads': totalLeads};
    for(const st of STATUSES){
      let pool = rows.filter(r => r.leadStatus===st);
      if(!isAllMonths()) pool = pool.filter(r => monthFilter(r[statusMonthCol(st)]));
      obj[st] = pool.length;
    }
...
function campaignByTeam(){
  ...
  let base = applyRefColdFilter(STATE.raw);
  if(!isAllMonths()) base = base.filter(r => monthFilter(r.CTM));
...
function incomeSegment(){
  ...
  const leadRows = isAllMonths() ? rows : rows.filter(r => monthFilter(r.CTM));
  const convRows = isAllMonths()
    ? rows.filter(r => r.leadStatus==='CONVERTED')
    : rows.filter(r => r.leadStatus==='CONVERTED' && monthFilter(r.CM));
  ...
  // known-only — don't push "other" values as individual rows
  ...
  // Blank Values row
  const blankLeads = leadRows.filter(r => !r.annualIncome || String(r.annualIncome).trim()==='');
  const blankConv  = convRows.filter(r => !r.annualIncome || String(r.annualIncome).trim()==='').length;
  results.push({ 'Income Band': 'Blank Values', Leads: blankLeads.length, Converted: blankConv,
    'Conv. Rate': blankLeads.length>0 ? blankConv/blankLeads.length : 0, _blank: true });

  // Other Values row (non-blank, non-known)
  const otherLeads = leadRows.filter(r => r.annualIncome && String(r.annualIncome).trim()!=='' && !orderSet.has(r.annualIncome));
  const otherConv  = convRows.filter(r => r.annualIncome && String(r.annualIncome).trim()!=='' && !orderSet.has(r.annualIncome)).length;
  results.push({ 'Income Band': 'Other Values', Leads: otherLeads.length, Converted: otherConv,
    'Conv. Rate': otherLeads.length>0 ? otherConv/otherLeads.length : 0, _other: true });
...
function costSummaryByCampaign(){
  ...
  for(let i=1;i<cpc.length;i++){
    ...
    if(isAllMonths()){
      ...
      qual  = STATE.raw.filter(r => r['Campaign Name']===name && (r.leadStatus==='CONVERTED'||r.leadStatus==='IN PROCESS')).length;
      cost = 0; for(let j=1;j<row.length;j++) cost += Number(row[j])||0;
    } else {
      leads = STATE.raw.filter(r => r['Campaign Name']===name && monthFilter(r.CTM)).length;
      qual  = STATE.raw.filter(r => r['Campaign Name']===name && monthFilter(r.CTM) && (r.leadStatus==='CONVERTED'||r.leadStatus==='IN PROCESS')).length;
      // sum cost across all selected months
      const selMonths = Array.isArray(STATE.filterMonth) ? STATE.filterMonth : [STATE.filterMonth];
      cost = selMonths.reduce((s,m) => { const idx=monthCols.indexOf(m); return s + (idx>=0?(Number(row[idx+1])||0):0); }, 0);
    }
...
function rmPerformance(){
  ...
  const revRowMatches = r => {
    if(month !== 'All' && revMonthOf(r) !== month) return false;
...
function renderRMPerformance(){
  ...
  if($('#rmperf-month-wrap')){
    buildMultiSelect('#rmperf-month-wrap', ['All',...rmMonths], STATE.rmPerfMonth,
      val => { STATE.rmPerfMonth = val; renderRMPerformance(); }, {multi:true});
  }
...
function renderStatusMonth(){
  if(!STATE.filesLoaded.fin23){ setNotUploaded('#tbl-status-month','fin23'); $('#legend-status-month').innerHTML=''; return; }
  const months = filteredMonths();
  const data = statusByMonth();
  applyHeat(data, 'total');
  renderHeatLegend('#legend-status-month', 'Row heat by Total');
  const rows = data.map(r => {
