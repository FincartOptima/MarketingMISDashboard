    ...
    for(const st of STATUSES){
      ...
      if(!isAllMonths()){
        if(mode==='AnyMonth'){
          ...
          const acol = anyMonthCol(st);
          pool = pool.filter(r =>
            (monthFilter(r[col]) && monthFilter(r.CTM)) ||
            (monthFilter(r[acol]) && !monthFilter(r.CTM))
          );
        }
      }
...
function landingPageStatusBreakdown(){
  ...
  const applyMode = (pool, col) => {
    if(isAllMonths()) return pool;
    if(mode==='AnyMonth')   return pool.filter(r => monthFilter(r[col]) && !monthFilter(r.CTM));
    if(mode==='SameMonth')  return pool.filter(r => monthFilter(r[col]) && monthFilter(r.CTM));
    return pool.filter(r => monthFilter(r[col]));
  };
  ...
  for(const lp of sortedLPs){
    ...
    for(const st of STATUSES){
      const pool = applyMode(sub.filter(r => r.leadStatus===st), dashboardStatusMonthCol(st));
      obj[st] = pool.length; total += pool.length;
    }
    obj.Total = total;
    obj.LCR  = total>0 ? obj.CONVERTED/total : 0;
...
const TABLE_INFO = {
  ...
  'platform-status': {
    ...
    desc: 'Lead counts by platform and status.<br><strong>Same Month:</strong> event-date column + CTM both in selected month(s).<br><strong>Any Month:</strong> CTM outside selected month(s), but CONVERTED uses CM, IN PROCESS uses LPM, all other statuses use LSM (Last Status Month) in selected month(s).<br><strong>Default:</strong> union of Same Month + Any Month (no double counting — CTM conditions are mutually exclusive).',
    cols: 'platformName · leadStatus · CTM (Created Month) · LSM (Last Status Month, for AnyMonth: ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · CM (Converted Month, for CONVERTED) · LPM (Lead In-Process Month, for IN PROCESS) · FMONTH (for SameMonth / Default same-month leg: non-CONVERTED/IN PROCESS)',
    source: 'RAW_DATA sheet',
    note: 'Any Month: CONVERTED uses CM, IN PROCESS uses LPM, all other statuses (ASSIGNED, RE-ASSIGNED, FOLLOW UP, ON HOLD, DEAD) use LSM. Default = Same Month ∪ Any Month — since Same Month requires CTM in the filter and Any Month requires CTM outside it, no lead is ever double-counted.'
  },
  'team-perf': {
  ...
  'campaign-team': {
    ...
    desc: 'Lead counts cross-tabulated by team (from firstRmName) and campaign. CTM used to apply the month filter.',
  ...
  'lp-status': {
    ...
    desc: 'Lead counts by landing page and status, filtered by campaign. Every row is assigned to exactly one landing page so all counts sum to the Grand Total.',
    cols: 'landingPage (all landing page values) · Campaign Name (selected campaign) · leadStatus (CONVERTED / IN PROCESS / ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · FMONTH (selected months, for non-CONVERTED/IN PROCESS) · LPM (selected months, for IN PROCESS) · CM (selected months, for CONVERTED) · CTM (selected months, Same Month / Any Month mode)',
    source: 'RAW_DATA sheet',
    note: 'In Default mode, IN PROCESS count uses LPM (Lead In-Process Month). RAW_DATA FMONTH for non-converted leads = CTM (Created Month). Leads created before the selected month that moved to IN PROCESS within it are counted here but will not appear in a RAW_DATA filter on FMONTH — causing a difference between this table\'s Grand Total and a RAW_DATA FMONTH filter. Use Same Month mode to match RAW_DATA CTM-based counts.'
  },
  'rmrev': {
    title: 'RM Revenue',
