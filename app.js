...
function teamPerformance(){
  ...
  return teams.map(team => {
    // Total Leads by firstRmName team
    const firstRmRows = base.filter(r => firstRmTeam(r) === team);
    const totalLeads = firstRmRows.filter(r => monthFilter(r.CTM)).length;
    // Status columns by currentRM team
    const currentRmRows = base.filter(r => (r.Team || 'SV') === team);
    const obj = {Team: team, 'Total Leads': totalLeads};
    for(const st of STATUSES){
      const pool = currentRmRows.filter(r => r.leadStatus===st && monthFilter(r[statusMonthCol(st)]));
      obj[st] = pool.length;
    }
...
function costPerLeadPerRMWithTotals(){
  ...
  // Total Leads and Total Cost counted by firstRmName
  const firstRmGroups = {};
  for(const r of scope){
    ...
    if(!firstRmGroups[key]) firstRmGroups[key] = {Team:frmTeam, RM:rm, totalLeads:0, totalCost:0};
    firstRmGroups[key].totalLeads++;
    firstRmGroups[key].totalCost += cplMap[r['Campaign Name']] || 0;
  }

  // Quality leads counted by currentRmName
  const groups = {};
  for(const r of scope){
  ...
  // Merge firstRM totals into groups
  for(const key of Object.keys(firstRmGroups)){
    if(!groups[key]){
      groups[key] = {Team:firstRmGroups[key].Team, RM:firstRmGroups[key].RM, totalLeads:0, totalCost:0, qualLeads:0};
    }
    groups[key].totalLeads = firstRmGroups[key].totalLeads;
    groups[key].totalCost = firstRmGroups[key].totalCost;
  }
...
function renderLandingPageStatus(){
  ...
  const headers = ['Landing Page', 'Campaign', ...STATUSES, 'Total', 'LCR', 'QLCR'];
  const rows = data.map(r => {
...
const TABLE_INFO = {
  ...
  'team-perf': {
    ...
    desc: 'Lead and status counts per team. Total Leads uses <strong>firstRmName</strong> (the RM first assigned the lead) to determine team. All status columns use <strong>currentRmName</strong> (current RM) to determine team. Total Leads uses CTM. Each status uses its own event-date column.',
    cols: 'firstRmName (→ team, for Total Leads) · currentRmName (→ team, for status columns) · leadStatus (CONVERTED / IN PROCESS / ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · CTM (selected months, for Total Leads / ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · LPM (selected months, for IN PROCESS) · CM (selected months, for CONVERTED)',
    source: 'RAW_DATA sheet, EMPLOYEE_REF sheet',
    note: 'Total Leads is based on firstRmName team; status columns are based on currentRmName team. If a lead was transferred between teams, it counts toward the original team in Total Leads but toward the current team in status columns. IN PROCESS uses LPM; Total Leads uses CTM — so status columns may not sum to Total Leads.'
  },
  'campaign-team': {
  ...
  'cpl-rm': {
    ...
    desc: 'Campaign costs split proportionally to each RM based on their share of that campaign\'s leads. Total Leads and Total Cost use <strong>firstRmName</strong> (the RM who was first assigned the lead). Quality Leads uses <strong>currentRmName</strong>. CPL = Total Cost / Total Leads; CPQL = Total Cost / Quality Leads.',
    cols: 'firstRmName (for Total Leads &amp; Total Cost) · currentRmName (for Quality Leads) · Campaign Name (all campaigns) · CTM (selected months) · leadStatus (CONVERTED / IN PROCESS, for Quality Leads)',
