...
const TABLE_INFO = {
  ...
  'lp-status': {
    ...
    desc: 'Lead counts by landing page and status, filtered by campaign. All modes use: CM for CONVERTED, LPM for IN PROCESS, LSM for all other statuses.<br><strong>Same Month:</strong> status column + CTM both in selected month(s).<br><strong>Any Month:</strong> status column in selected month(s) but CTM outside.<br><strong>Default:</strong> status column in selected month(s) regardless of CTM.',
    cols: 'landingPage · Campaign Name · leadStatus · CTM (Created Month) · LSM (Last Status Month, for ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · CM (Converted Month, for CONVERTED) · LPM (Lead In-Process Month, for IN PROCESS)',
    source: 'RAW_DATA sheet',
    note: 'FMONTH is not used in this table. CONVERTED uses CM, IN PROCESS uses LPM, all other statuses use LSM. Default = Same Month ∪ Any Month — no double counting.'
  },
  'rmrev': {
    title: 'RM Revenue',
