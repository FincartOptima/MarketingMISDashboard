...
function initFilters(){
  ...
  ['All', ...months].forEach(m => { const o=document.createElement('option'); o.value=m; o.textContent=m; sel.appendChild(o); });
  sel.value = months.includes(STATE.filterMonth) || STATE.filterMonth==='All' ? STATE.filterMonth : 'All';
...
function campaignByTeam(){
  ...
  const refMode = STATE.filterRefCold;
  let base = applyRefColdFilter(STATE.raw);
  if(month !== 'All') base = base.filter(r => r.CTM === month);
  ...
  const rows = teams.map(team => {
    ...
    for(const c of campaigns){
      const n = base.filter(r => r.Team===team && r['Campaign Name']===c).length;
      obj[c] = n; total += n;
    }
    obj.Total = total;
    return obj;
...
function costSummaryByCampaign(){
  ...
  for(let i=1;i<cpc.length;i++){
    ...
    if(month==='All'){
      ...
      qual  = STATE.raw.filter(r => r['Campaign Name']===name && r.CTM===month && (r.leadStatus==='CONVERTED'||r.leadStatus==='IN PROCESS')).length;
...
function renderRMPerformance(){
  ...
  if(monthSel){
    ...
    monthSel.innerHTML = ['All', ...rmMonths].map(m=>`<option value="${m}">${m}</option>`).join('');
    monthSel.value = (rmMonths.includes(STATE.rmPerfMonth)||STATE.rmPerfMonth==='All') ? STATE.rmPerfMonth : 'All';
...
function renderCampaignByTeam(){
  ...
  renderTable('#tbl-campaign-team', headers, data);
}
...
function parseCsv(text){
  ...
  // Cast numeric cells from col 1 onwards
  return rows.filter(r=>r.some(c=>c!=='')).map((r,ri)=>{
    if(ri === 0) return r;
    return r.map((v,ci)=> ci===0 ? v : (v===''?0: (isNaN(+v)?v:+v)) );
  });
}
...
async function tryAutoLoad(){
  ...
  // Try GitHub auto-load (configured in Settings)
  const s = loadSettings();
  const ghUrl = ghRawUrl(s);
  if(ghUrl){
    try{
    ...
    } catch(e){ console.warn('GitHub auto-load failed:', e); }
  }
