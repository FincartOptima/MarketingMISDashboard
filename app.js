...
function campaignByTeam(){
  ...
  const teamSet = new Set(base.map(r => r.Team || 'SV'));
  ...
  const rows = teams.map(team => {
    ...
    for(const c of campaigns){
      const n = base.filter(r => (r.Team || 'SV')===team && r['Campaign Name']===c).length;
      obj[c] = n; total += n;
    }
    obj.Total = total;
    return obj;
...
function costPerLeadPerRMWithTotals(){
  ...
  // Total Leads, Total Cost, and Quality Leads all counted by currentRmName
  const groups = {};
  for(const r of scope){
