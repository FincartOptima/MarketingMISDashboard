...
function topKPIs(){
  ...
  const ytd = rows.filter(r=>monthFilter(r.FMONTH)).length;
  ...
  for(const g of groups){
    ...
    for(const st of STATUSES){
      ...
      if(!isAllMonths()){
        ...
        if(mode==='AnyMonth'){
          pool = pool.filter(r => monthFilter(r[col]) && monthKey(r.CTM) < Math.min(...effectiveMonths().map(m=>monthKey(m))));
...
const TABLE_INFO = {
  ...
  'platform-status': {
    ...
    desc: 'Lead counts by platform and status. Default mode: each status uses its own event-date column. Same Month: both event-date and CTM must be in selected months. Any Month: event-date in selected months but CTM outside.',
    cols: 'platformName (Google Adwords / Facebook / Brand Marketing / BTL Marketing / Referral / Emailer / Direct Registration / Cold Leads) · leadStatus (CONVERTED / IN PROCESS / ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · FMONTH (selected months, for non-CONVERTED/IN PROCESS) · LPM (selected months, for IN PROCESS) · CM (selected months, for CONVERTED) · CTM (selected months, Same Month / Any Month mode)',
    source: 'RAW_DATA sheet',
    note: 'In Default mode, IN PROCESS count uses LPM (Lead In-Process Month). RAW_DATA FMONTH for non-converted leads = CTM (Created Month). Leads created before the selected month that moved to IN PROCESS within it are counted here but will not appear in a RAW_DATA filter on FMONTH. Use Same Month mode to restrict both creation and status event to the selected month.'
  },
  'team-perf': {
