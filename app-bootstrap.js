...
const TABLE_INFO = {
  ...
  'platform-status': {
    ...
    desc: 'Lead counts by category and status. <strong>Category Names are dynamically read from uploaded data</strong> — new categories are automatically included. CONVERTED uses CM, IN PROCESS uses LPM, all other statuses (ASSIGNED, RE-ASSIGNED, FOLLOW UP, ON HOLD, DEAD) use CTM.<br><strong>Same Month:</strong> status column matches selected month(s) AND CTM matches.<br><strong>Any Month:</strong> status column matches selected month(s) but CTM outside (only applies to CONVERTED/IN PROCESS; other statuses will be 0 since their column IS CTM).<br><strong>Default:</strong> status column matches selected month(s).<br><br>The <strong>Lead Head</strong> dropdown filters by the B2C file\'s leadHead column (currently Primary/Secondary) — options are read dynamically from the uploaded data, same as Category Name. <strong>All Status</strong> (the default) includes every leadHead value.',
    cols: 'Campaign Name / Category Name (dynamic, all unique values — the source\'s "Category Name" and "Campaign Name" columns map onto the same field) · leadHead (Lead Head filter) · leadStatus · CTM (Created Month, used for ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · CM (Converted Month, for CONVERTED) · LPM (Lead In-Process Month, for IN PROCESS)',
    source: 'RAW_DATA sheet',
