...
const TABLE_INFO = {
  ...
  'bdperf': {
    ...
    desc: 'Each rep\'s leads live in a sheet named "&lt;Person&gt; Q&lt;N&gt;" (e.g. "Himani Q1") in the BD Accountability Tracker upload — new quarters are picked up automatically once a sheet with that name exists, no code change needed.<br><br><strong>Team</strong> is resolved from the sheet\'s "Team Leader" column (e.g. "Yash Tawri" → team "Yash T"), via the same RM Master Mapping / EMPLOYEE_REF data the main dashboard uses — so a name fix there also improves BD team resolution. Blank or unrecognized Team Leader values fall back to SV.<br><br>By default the table shows one row per <strong>Team</strong>; selecting a single team in the Team filter drills down to one row per <strong>Assigned RM</strong> within it — same pattern as the Dashboard tab\'s Team Performance Matrix. <strong>Person</strong> (which BD rep sourced the lead) and <strong>Quarter</strong> are independent filters layered on top and also narrow the three charts above the table.<br><br>Conv. Rate = CONVERTED ÷ Total. GMeet Joined / Financial Plan Created count rows marked "Yes" only — the source sheets also use "Pending" and blank, both excluded from these Yes-only counts (and from the pie charts).',
    cols: 'Sheet name (→ Person, Quarter) · Team Leader (→ Team) · Assigned RM · Current Stage (LEAD ASSIGNED / IN FOLLOW-UP / IN PROCESS / CONVERTED / ON HOLD/DEAD / DROPPED / TAX FILING DONE) · GMeet Joined? (Yes/No/Pending) · Financial Plan Created? (Yes/No/Pending)',
    source: 'BD Accountability Tracker file, EMPLOYEE_REF sheet, RM Master Mapping sheet',
    note: 'Current Stage and Yes/No/Pending values are normalized case- and whitespace-insensitively from the raw sheets (e.g. "assign"/"Lead Assigned" both become LEAD ASSIGNED) — see BD_STAGE_MAP in backend/extract_lib.py. Team Leader misspellings ("Akanksha Aggrawal", "Vidhi Goyal") are corrected via RM Master Mapping entries.'
  },
  'lp-status': {
