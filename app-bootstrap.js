...
const TABLE_INFO = {
  ...
  'bdcallflow': {
    ...
    desc: 'The BD funnel end to end: <strong>Total Calls Made</strong> splits into <strong>Calls Connected</strong> and <strong>CNP</strong> (not picked); connected calls produce <strong>Meetings Scheduled</strong>; and scheduled meetings end up <strong>Joined</strong>, <strong>Not Joined</strong>, or <strong>Pending</strong>.<br><br><strong>Read the last row carefully — it is a different source from the rest.</strong> Calls Made / Connected / CNP / Meetings Scheduled are the BD reps\' own daily tallies from the <strong>BD Daily Log</strong> sheet. Joined / Not Joined / Pending come from the per-lead <strong>GMeet Joined?</strong> column in the "&lt;Person&gt; Q&lt;N&gt;" tracker sheets. Those two records are maintained separately by the BD team and do <em>not</em> reconcile — in a recent upload the Daily Log recorded ~1,530 meetings booked while the tracker carried a GMeet status on ~1,814 leads. The dashboard deliberately shows both as-is rather than scaling one to match the other, so treat the bottom row as "of the meetings the tracker knows about" rather than a strict subdivision of the box above it.<br><br>Because the first four stages come from a sheet with no per-lead linkage, the <strong>entire</strong> flow is scoped to <strong>Month + Person only</strong> — Team and Platform are ignored here even when set, so that every stage stays filtered identically. (Applying them to just the GMeet row would silently desync it from the stages above.)<br><br>The percentage on each box is its share of the stage it came from: Connected/CNP ÷ Calls Made, Meetings ÷ Connected, and each GMeet state ÷ the tracker\'s total meetings with a status.',
    cols: 'Total Calls Made · Calls Connected · CNP · Appointments Booked (→ Meetings Scheduled) — all from BD Daily Log · GMeet Joined? Yes/No/Pending — from the tracker sheets',
    source: 'BD Accountability Tracker file: "BD Daily Log" sheet + the "<Person> Q<N>" lead sheets',
