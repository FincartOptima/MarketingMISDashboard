...
const TABLE_INFO = {
  ...
  'platform-status': {
    ...
    desc: 'Lead counts by category and status. <strong>Category Names are dynamically read from uploaded data</strong> — new categories are automatically included. CONVERTED uses CM, IN PROCESS uses LPM, all other statuses (ASSIGNED, RE-ASSIGNED, FOLLOW UP, ON HOLD, DEAD) use CTM.<br><strong>Same Month:</strong> status column matches selected month(s) AND CTM matches.<br><strong>Any Month:</strong> status column matches selected month(s) but CTM outside (only applies to CONVERTED/IN PROCESS; other statuses will be 0 since their column IS CTM).<br><strong>Default:</strong> status column matches selected month(s).<br><br>This table (like every other table on the Dashboard tab) also honors the <strong>Lead Head</strong> filter in the header above — it filters by the B2C file\'s leadHead column (options read dynamically from the uploaded data), defaults to <strong>Primary</strong> so an Additional/secondary-applicant row on the same lead isn\'t double-counted, and can be set to All to remove the restriction.',
    cols: 'Campaign Name / Category Name (dynamic, all unique values — the source\'s "Category Name" and "Campaign Name" columns map onto the same field) · leadHead (Lead Head filter, in the header) · leadStatus · CTM (Created Month, used for ASSIGNED / RE-ASSIGNED / FOLLOW UP / ON HOLD / DEAD) · CM (Converted Month, for CONVERTED) · LPM (Lead In-Process Month, for IN PROCESS)',
    source: 'RAW_DATA sheet',
  ...
  'rm-transfer': {
    ...
    desc: 'Shows leads that were transferred from one RM to another (where firstRmName ≠ currentRmName). The summary row shows how many leads moved <strong>out</strong> of each team (based on firstRmName\'s team) and lists the top destination teams.<br><br>Click a team row to expand RM-level details: the original RM (FirstRM), campaign, transfer count, destination RM (CurrentRM), destination team, and lead status.<br><br>Three blocks below it count <strong>every</strong> lead, not just the transferred ones, split into <strong>Same Team</strong> (the lead\'s current Team still equals firstRmName\'s team — includes a handoff between two RMs on the same team, deliberately looser than "same RM" per a 2026-09 comparison against a manually-built reference pivot), <strong>Transferred to SV</strong> (current Team is SV and it isn\'t Same Team — covers every generic placeholder currentRmName under that team, e.g. "Support Wealth Manager", not just a literal currentRmName of "SV"), and <strong>Transferred to another team</strong> (anything else): first the overall totals, then the same split by <strong>Category Name</strong> (Branding, Corporate, Google, Social Media, etc.), then by the <strong>originating team</strong> (firstRmName\'s team, same "From Team" grouping as the main table). <strong>Retained %</strong> = Same Team ÷ Total for that row. These three blocks also treat "All Months" as literally every lead ever (no Apr-2026 cutoff), unlike the main table above and every other month-wise table on this dashboard.',
    ...
    source: 'RAW_DATA sheet, EMPLOYEE_REF sheet',
    note: 'Main table: only leads where firstRmName ≠ currentRmName (case-insensitive) are counted; if both are blank, the lead is excluded. Honors global Month, Ref+Cold, Status, and Lead Head filters. The three summary blocks below it honor Ref+Cold/Status/Lead Head but not the FY-scoped Month convention — see above.'
  },
  'campaign-team': {
  ...
  'bdperf': {
    ...
    desc: 'Each rep\'s leads live in a sheet named "&lt;Person&gt; Q&lt;N&gt;" (e.g. "Himani Q1") in the BD Accountability Tracker upload — new quarters are picked up automatically once a sheet with that name exists, no code change needed. Only <strong>Date Assigned</strong>, <strong>Email</strong>, <strong>GMeet Joined?</strong>, and <strong>Current Stage</strong> are read from the tracker itself.<br><br>Every lead is matched to its B2C record by email (BD tracker) &lt;-&gt; userId (B2C — despite the name, it\'s the client\'s email, and is unique across the file). For a matched lead, <strong>Stage</strong>, <strong>RM</strong>, <strong>Team</strong>, <strong>Month</strong>, and <strong>Platform</strong> all come from that B2C record (Stage uses the same CM-presence CONVERTED rule as the rest of the dashboard, not just leadStatus) — note this means RM/Team reflect who <em>currently</em> owns the lead in the CRM, not who the tracker originally assigned it to; leads that have been reassigned since will show under their new owner. A lead with no match (not yet in the CRM, or an email typo) is labeled "(Not in CRM)" under team SV, excluded from the Stage breakdown, and its Month falls back to the tracker\'s own Date Assigned.<br><br>By default the table shows one row per <strong>Team</strong>; selecting a single team in the Team filter drills down to one row per <strong>RM</strong> within it — same pattern as the Dashboard tab\'s Team Performance Matrix. A <strong>Workpoint Status / BD Team Status</strong> toggle above the table switches which Stage classification the columns tally: <strong>Workpoint Status</strong> (default) uses the B2C-derived Stage described above; <strong>BD Team Status</strong> instead uses the tracker\'s own self-reported Current Stage column, unchanged since the BD rep last updated it — the two can disagree once a lead is reassigned or its CRM status moves on after the tracker was last updated. The <strong>Current Stage</strong> bar chart above follows this same toggle (see the label under its heading), switching its bars between the two classifications along with the table. Click any column header to sort. <strong>Person</strong> (which BD rep sourced the lead), <strong>Month</strong>, and <strong>Platform</strong> are independent filters layered on top and also narrow the charts and the GMeet-vs-In-Process table below them; <strong>Reset filters</strong> clears all of them (including the GMeet pie\'s click-filter) back to All in one click — it does not affect the Workpoint/BD Team Status toggle.<br><br>The separate <strong>GMeet Joined? vs In Process</strong> table always shows all three GMeet states side by side — it deliberately ignores the pie chart\'s own click-filter (collapsing to one state would defeat a table meant to compare them) but still honors Month/Team/Person/Platform. Unlike the Team/RM table, it is always sourced from the tracker\'s own Current Stage column (the BD Team\'s Google Sheet), regardless of the toggle above, since it is meant to reflect what BD ops themselves logged.<br><br>Conv. Rate = CONVERTED ÷ Total. QL Conv. Rate = (CONVERTED + IN PROCESS) ÷ Total. In Process Rate (GMeet table only) = rows with tracker Current Stage = IN PROCESS ÷ Total. GMeet Joined counts rows marked "Yes" only — the tracker also uses "Pending" and blank, both excluded from this Yes-only count (and from the pie chart).',
    cols: 'Sheet name (→ Person) · Email (→ matched to B2C userId) · GMeet Joined? (Yes/No/Pending) · Date Assigned (Month fallback for unmatched leads) · Current Stage (tracker\'s own field → BD Team Status, GMeet-vs-In-Process table) · B2C leadStatus/CM (→ Stage / Workpoint Status) · B2C currentRmName (→ RM) · B2C Team (→ Team) · B2C CTM (→ Month) · B2C platformName (→ Platform)',
    source: 'BD Accountability Tracker file (incl. its own Current Stage column), RAW_DATA (B2C) sheet, RM Master Mapping sheet',
  ...
  'bdcalls': {
    title: 'BD Daily Log — Call Activity',
    desc: 'Reads the <strong>BD Daily Log</strong> sheet in the same BD Accountability Tracker upload as the rest of this tab — one row per BD rep per day, with no per-lead detail at all (it\'s pure call-activity volume, not tied to individual clients). Because of that it has no Team or Platform of its own, so only the <strong>Month</strong> and <strong>Person</strong> filters at the top of the tab apply to it; Team/Platform are ignored here.<br><br><strong>Total Calls Made</strong>, <strong>Calls Connected</strong>, and <strong>CNP</strong> (Calls Not Picked) are read directly from their own columns and shown as-is — CNP is <em>not</em> recomputed as Total minus Connected. In this upload a small number of rows (about 1 in 20) don\'t add up exactly, and a couple are clearly data-entry errors (CNP larger than Total Calls on that day) — rather than silently "fixing" someone\'s logged numbers, the dashboard shows exactly what\'s in the sheet and lets Connected%/CNP% be whatever the raw numbers produce for those rows.<br><br>The chart aggregates by month (three bars side by side — Made / Connected / CNP) since 100+ individual daily bars would be unreadable; the table breaks the same filtered rows down by BD rep instead, so the two are complementary views of the same underlying rows rather than duplicates of each other.<br><br><strong>Meetings Scheduled</strong> is <em>not</em> shown in this table — it\'s tracker-sourced (Date Assigned, not this sheet), so it lives in the Call Flow diagram/table above instead. See that section\'s info button for why.',
    cols: 'Date (→ Month, via the shared Month filter) · BD Name (→ Person, via the shared Person filter — same rep names as the rest of the tab) · Total Calls Made · Calls Connected · CNP',
    source: 'BD Accountability Tracker file, "BD Daily Log" sheet',
    note: 'Rows with a blank Date or blank BD Name are dropped during extraction — the source sheet has a few scratch/placeholder rows (an unlabelled future-quarter row, a hidden grand-total row) that carry numbers but aren\'t real daily entries.'
  },
  'bdcallflow': {
    title: 'Call Flow — Calls to Meetings Joined',
    desc: 'The BD funnel end to end: <strong>Total Calls Made</strong> splits into <strong>Calls Connected</strong> and <strong>CNP</strong> (not picked); connected calls lead to <strong>Meetings Scheduled</strong>; and those meetings end up <strong>Joined</strong>, <strong>Not Joined</strong>, <strong>Pending</strong>, or <strong>Not Logged</strong>.<br><br><strong>Meetings Scheduled</strong> is every lead in the "&lt;Person&gt; Q&lt;N&gt;" tracker sheets whose <strong>Date Assigned</strong> falls in the selected month(s) — every lead handed to a BD rep that period counts as a meeting scheduled with them (verified against the source file: Suhani\'s 27 leads assigned in September match the BD team\'s own day-by-day count exactly). <strong>Joined / Not Joined / Pending / Not Logged</strong> is that <em>exact same set of leads</em>, broken down by their own <strong>GMeet Joined?</strong> column (Not Logged = no value entered there yet) — so those four always add up to Meetings Scheduled precisely, with nothing left over.<br><br>The one place two different sheets meet is <strong>Calls Connected → Meetings Scheduled</strong>: Calls Made/Connected/CNP are the BD reps\' own daily tallies in the <strong>BD Daily Log</strong> sheet, while Meetings Scheduled is counted from the tracker sheets instead. Treat that specific percentage as directional (roughly how many connected calls turn into a scheduled meeting) rather than a strict conversion, since the two sides aren\'t the same underlying records.<br><br>Because Meetings Scheduled has no Team/Platform of its own to filter by, the <strong>entire</strong> flow — including the top three stages — is scoped to <strong>Month + Person only</strong>; Team and Platform are ignored here even when set, so every stage stays filtered identically. (Applying them to only part of the flow would silently desync one stage from the rest.)<br><br>The percentage on each box is its share of the stage it came from: Connected/CNP ÷ Calls Made, Meetings ÷ Connected, and each GMeet state ÷ Meetings Scheduled.',
    cols: 'Total Calls Made · Calls Connected · CNP — from BD Daily Log · Date Assigned (→ Meetings Scheduled) · GMeet Joined? Yes/No/Pending/blank — from the tracker sheets',
